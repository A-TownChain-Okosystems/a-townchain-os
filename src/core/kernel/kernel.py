...
class ShivaKernel:
    ...
    def __init__(self):
        ...
        self.capabilities = CapabilityManager()   # Milestone (08.07.2026): Capability-Durchsetzung
        self._running    = False
    ...
    def alloc(self, size: int, pid: int) -> MemRegion:
        """Speicher für Prozess allokieren. Vergibt automatisch eine
        Capability (ALL rights) an den anfordernden Prozess — Milestone
        08.07.2026, additiv, bricht bestehende Aufrufer nicht."""
        ...
        cap = self.capabilities.grant(
            owner_pid=pid, resource_type=ResourceType.MEMORY,
            resource_id=id(region), rights=Right.ALL, issued_by=0)
        region._cap_id = cap.cap_id
        self._log(f"ALLOC pid={pid} size={size} cap={cap.cap_id[:8]}")
        return region

    def free(self, region: MemRegion) -> bool:
        cap_id = getattr(region, "_cap_id", None)
        if cap_id:
    ...
    def read_memory(self, cap_id: str, region: MemRegion, offset: int, length: int) -> bytes:
        """Capability-gesicherter Speicherzugriff. Wirft CapabilityError
        ohne gueltige READ-Capability fuer diese Region."""
        self.capabilities.require(cap_id, ResourceType.MEMORY, id(region), Right.READ)
        return region.read(offset, length)

    def write_memory(self, cap_id: str, region: MemRegion, offset: int, data: bytes) -> None:
        """Capability-gesicherter Speicherzugriff. Wirft CapabilityError
        ohne gueltige WRITE-Capability fuer diese Region."""
        self.capabilities.require(cap_id, ResourceType.MEMORY, id(region), Right.WRITE)
    ...
    def create_channel(self, ptype: ChannelType = ChannelType.QUEUE,
        ...
        # Milestone 08.07.2026: Sender bekommt automatisch eine Capability
        # fuer den neuen Kanal (additiv, alte Aufrufer unberuehrt).
        self.capabilities.grant(
            owner_pid=sender_pid, resource_type=ResourceType.IPC_CHANNEL,
            resource_id=cid, rights=Right.ALL, issued_by=0)
        self._log(f"CHAN_CREATE cid={cid} type={ptype.name}")
        return cid

    def send_with_capability(self, cap_id: str, cid: int, from_pid: int,
                             msg_type: str, data: Any) -> bool:
        """Capability-gesichertes Senden. Wirft CapabilityError ohne
        gueltige WRITE-Capability fuer den Kanal."""
        self.capabilities.require(cap_id, ResourceType.IPC_CHANNEL, cid, Right.WRITE)
        return self.channel_send(cid, from_pid, msg_type, data)

    def recv_with_capability(self, cap_id: str, cid: int, pid: int) -> Optional[IPCMessage]:
        """Capability-gesichertes Empfangen. Wirft CapabilityError ohne
        gueltige READ-Capability fuer den Kanal."""
        self.capabilities.require(cap_id, ResourceType.IPC_CHANNEL, cid, Right.READ)
        return self.channel_recv(cid, pid)
